  ...
  return (
    <motion.div layout initial={{ borderRadius: 10 }} className={styles.section}>
      <div className={styles.section_top}>
        ...
        <div className={styles.section_btn} onClick={toggleOpen}>
...
function ProductCustomPage() {
  ...
  const generateRandomAvatar = () => {
    ...
    // 최종 캐릭터
    let dicebearUrl = `https://api.dicebear.com/7.x/adventurer/svg?flip=true&eyebrows=${`variant${String(
      randomEyebrows,
  ...
  const marketProduct = async () => {
    ...
    // 첫 번째 문장
    let response1 = await axios.get(`https://j9e105.p.ssafy.io/ai2/synthesize?voice_id=${voiceId}&text=${text1}`, {
      responseType: "blob",
    });
    let url1 = await uploadFile(response1.data, text1);

    // 두 번째 문장
    let response2 = await axios.get(`https://j9e105.p.ssafy.io/ai2/synthesize?voice_id=${voiceId}&text=${text2}`, {
      responseType: "blob",
    });
    let url2 = await uploadFile(response2.data, text2);

    // 세 번째 문장
    let response3 = await axios.get(`https://j9e105.p.ssafy.io/ai2/synthesize?voice_id=${voiceId}&text=${text3}`, {
      responseType: "blob",
    });
          ...
          <div>
            <Section title="기본 정보">
              <div className={styles.section1}>
                <div className={`${styles.input} ${styles.title}`}>
                  상품 제목 : <input type="text" value={title} onChange={handleTitleChange} />
                </div>
            ...
            <Section title="가격 설정">
              <div className={styles.section2}>
                <div className={styles.input}>
                  상품 가격 : <input type="text" value={price} onChange={handlePriceChange} /> / 원 <span>(단위)</span>
                </div>
              </div>
            </Section>
              ...
              <div className={styles.section4}>
                <div className={styles.input}>
                  ...
                  <input
                    ...
                    maxLength={mL}
                  />
                  <span className={styles.smallText}>{`${text1Length}/20자`}</span>
                </div>
                <div className={styles.input}>
                  문장 2 :{" "}
                  <input
                    ...
                    maxLength={mL}
                  />
                  <span className={styles.smallText}>{`${text2Length}/20자`}</span>
                </div>
                <div className={styles.input}>
                  문장 3 :{" "}
                  <input
                    ...
                    maxLength={mL}
                  />
                  <span className={styles.smallText}>{`${text3Length}/20자`}</span>
                </div>
              </div>
            </Section>
          </div>
